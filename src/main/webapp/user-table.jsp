...
<%
UserEntity mySelf = (UserEntity) session.getAttribute("user");
%>
<!DOCTYPE html>
<html lang="en">
									...
									<tbody>
										<c:forEach var="user" items="${listUsers}" varStatus="status">
											<tr>
												...
												<td>
												<%
													if (mySelf != null && mySelf.getRoleID() == 1) {
													%>
												<a
													href="${pageContext.request.contextPath}/user-edit?id=${user.id}"
													class="btn btn-sm btn-primary">Sửa</a> <a
													href="${pageContext.request.contextPath}/users?deleteID=${user.id}"
													class="btn btn-sm btn-danger">Xóa</a> 
													<%
													}
													%>
													<a
													href="${pageContext.request.contextPath}/user-details?id=${user.id}"
